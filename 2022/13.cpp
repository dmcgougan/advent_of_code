...
static vector<pkt> parse_r()
{
    ...
    if (pos + 1 < pstr.length() && pstr[++pos] == ',') pos++;

    return l;
}

static pkt parse(const string& s)
{
    pstr = s;
    pos = 0;
    return pkt{parse_r()};
...
static bool same(const pkt& left, const pkt& right)
{
    ...
    if (!left.isInt() && !right.isInt() && left.l.size() == right.l.size()) {
        ...
        while (liter != left.l.end()) {
            if (!same(*liter++, *riter++)) return false;
        }
        return true;
    }
    return false;
...
static ll order(const pkt& left, const pkt& right)
{
    if (left.isInt() && right.isInt()) {
    ...
    if (!left.isInt() && !right.isInt()) {
        ...
        if (liter == left.l.end() && riter == right.l.end()) return 0;
...
int main()
{
    ...
    do {
        ...
        if (order(left, right) < 0) sum += index;
        index++;
