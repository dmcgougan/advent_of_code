...
static vector<vector<ll>> dijkstra(const vector<string>& terrain, ll destX, ll destY)
{
    ...
    while (locationsLeft > 0) {
        ...
        if (y > 0 && !costDone[y - 1][x] && terrain[y - 1][x] + 1 >= elevation &&
            cost[y - 1][x] > minDist + 1)
        {
            cost[y - 1][x] = minDist + 1;
        }
        if (y + 1 < height && !costDone[y + 1][x] && terrain[y + 1][x] + 1 >= elevation &&
            cost[y + 1][x] > minDist + 1)
        {
            cost[y + 1][x] = minDist + 1;
        }
        if (x > 0 && !costDone[y][x - 1] && terrain[y][x - 1] + 1 >= elevation &&
            cost[y][x - 1] > minDist + 1)
        {
            cost[y][x - 1] = minDist + 1;
        }
        if (x + 1 < width && !costDone[y][x + 1] && terrain[y][x + 1] + 1 >= elevation &&
            cost[y][x + 1] > minDist + 1)
        {
            cost[y][x + 1] = minDist + 1;
        }
...
static void printGuide(const vector<vector<ll>>& cost)
{
    ll height = cost.size();
